...
fn deserialize_with_context_test(
    expected_object: ProgramObject,
    expected_code: Code,
    input: Vec<u8>,
) {
...
fn serialize_test<S>(expected: Vec<u8>, object: S)
where
    S: Serializable,
{
...
fn serialize_with_context_test<S>(expected: Vec<u8>, object: S, code: Code)
where
    S: SerializableWithContext,
{
    let mut actual: Vec<u8> = Vec::new();
...
#[test]
fn deserialize_call_function() {
    let expected =
        OpCode::CallFunction { name: ConstantPoolIndex::new(1), arguments: Arity::new(2) };
...
fn serialize_string() {
    let expected = vec![
        0x02, 0x0C, 0x00, 0x00, 0x00, 0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0x57, 0x6F, 0x72, 0x6C,
        0x64, 0x0A,
    ];
...
#[test]
fn serialize_class() {
    let expected = vec![0x05, 0x02, 0x00, 0x2A, 0x00, 0x9A, 0x02];
    let object =
...
fn serialize_method() {
    let expected =
        vec![0x03, 0xFF, 0x00, 0x03, 0x0F, 0x00, 0x02, 0x00, 0x00, 0x00, 0x01, 0x2A, 0x00, 0x0F];
...
fn class() {
    let expected =
        ProgramObject::Class(vec![ConstantPoolIndex::new(42), ConstantPoolIndex::new(666)]);
    let bytes = vec![0x05, 0x02, 0x00, 0x2A, 0x00, 0x9A, 0x02];
    deserialize_with_context_test(expected, Code::new(), bytes);
}
...
fn method() {
    ...
    let code =
        Code::from(vec![OpCode::Literal { index: ConstantPoolIndex::new(42) }, OpCode::Return]);
